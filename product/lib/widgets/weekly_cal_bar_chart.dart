...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Weekly calorie bar chart',
      theme: ThemeData(
        primarySwatch: Colors.blue,
      ),
      home: Scaffold(
        appBar: AppBar(title: const Text('Weekly calorie bar chart')),
        body: const Center(child: WeeklyCalBarChart(title: 'Title')),
      ),
    );
  }
}

class WeeklyCalBarChart extends StatefulWidget {
  const WeeklyCalBarChart({super.key, required this.title});

  final String title;

  @override
  State<WeeklyCalBarChart> createState() => _WeeklyCalBarChart();
}

class _WeeklyCalBarChart extends State<WeeklyCalBarChart> {
  // 選択された棒グラフ（初期値は今日）
  int selectedIndex = 6;
                    ...
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        ...
                        getTitlesWidget: (double value, TitleMeta meta) {
                          ...
                          var formattedDate = datas[value.toInt()].getDayString();
